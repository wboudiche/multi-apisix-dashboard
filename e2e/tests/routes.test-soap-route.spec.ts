...
// what APISIX compares `http_soapaction` against.
...
const createRoute = async (data: Parameters<typeof postRouteReq>[1]) => {
  const res = await postRouteReq(e2eReq, data);
  createdRouteIds.push(res.data.value.id);
};

const openTestDrawer = async (page: Page, routeName: string) => {
  // Filter by the unique route name so leftover routes from other specs (the
  // list paginates at 10) can't push this one off the visible page.
  await page.goto(
    `${env.E2E_TARGET_URL.replace(/\/$/, '')}/routes?name=${routeName}`
  );
...
test('Route seeds the SOAPAction header for a per-operation SOAP route', async ({
  ...
  await createRoute({
    ...
    methods: ['POST'],
    vars: [['http_soapaction', '==', SOAP_ACTION]],
