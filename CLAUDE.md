...
- **`JWT_SECRET` default** is the string `"your-secret-key-change-in-production"`. The backend does not refuse to start with the default — easy hardening: panic in non-dev mode.
- **`X-Team-ID` is client-controlled.** The backend trusts whichever team the browser sends. Double-check that handlers verify the user actually belongs to that team before scoping queries by it.
