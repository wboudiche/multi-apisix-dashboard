...
// an empty list. The backend always holds at least its admin, so an empty read
// is a failed one, and judged against it every assignment would look orphaned.
...
func (s *MaintenanceService) FindOrphanedUserInstances(ctx context.Context) ([]OrphanedAssignment, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not read instance assignments: %w", err)
	}
	users, err := s.etcd.List(ctx, models.KeyPrefixUsers)
	if err != nil {
		return nil, fmt.Errorf("could not read users: %w", err)
	}
	if len(users) == 0 {
		return nil, ErrNoUsersRead
	}
	return orphanedAssignments(assignments, users), nil
}

// PurgeOrphanedUserInstances deletes the given assignment keys, each only if it
// is still orphaned when the purge runs. Keys that are not, or are not
// assignment keys at all, are skipped and said so.
func (s *MaintenanceService) PurgeOrphanedUserInstances(ctx context.Context, keys []string) (*PurgeResult, error) {
	orphans, err := s.FindOrphanedUserInstances(ctx)
	if err != nil {
		return nil, err
	}
	...
	for _, key := range keys {
		if !orphaned[key] {
			result.Skipped[key] = "not an orphaned instance assignment"
			continue
		}
...
// keys and values under /user_instances/ and /users/.
//
...
func orphanedAssignments(assignments, users map[string][]byte) []OrphanedAssignment {
