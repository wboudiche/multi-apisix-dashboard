...
func (h *MaintenanceHandler) ListOrphans(c *gin.Context) {
	...
	orphans, err := h.maintenance.FindOrphanedUserInstances(c.Request.Context())
	if err != nil {
		c.JSON(orphanReadStatus(err), gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"user_instances": orphans})
}
...
func (h *MaintenanceHandler) PurgeOrphans(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil || len(req.UserInstances) == 0 {
	...
	c.JSON(status, gin.H{"user_instances": result})
}
